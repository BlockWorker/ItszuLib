...
trait TileMultiFluidTank extends TileEntityBase with IFluidHandler {
  ...
  override def getTankInfo(from: EnumFacing): Array[FluidTankInfo] = {for (i <- 0 until tanks.length) yield tanks(i).getInfo}.toArray
  ...
  override def serverUpdate(): Unit = {
    ...
    for (i <- 0 until tanks.length) {
      val tank = tanks(i)
      PacketHandler.INSTANCE.sendToDimension(new MessageFluidTankUpdate(getPos.getX, getPos.getY, getPos.getZ, i, if (tank.getFluid == null) -1 else tank.getFluid.getFluidID, tank.getFluidAmount), getWorld.provider.getDimensionId)
    }
    updateNeeded = false
  }
